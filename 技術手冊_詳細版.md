...
| `ragic_client.py` | 從 Ragic API 擷取資料，支援分頁與重試。 |
| `data_transformer.py` | 將資料從 Ragic 格式轉換為 BigQuery 格式，處理欄位對照與型別轉換。 |
| `bigquery_uploader.py` | 將資料上傳至 BigQuery，支援多種上傳模式（MERGE、Staging+SP）。 |
| `email_notifier.py` | 從 Cloud Logging 取得日誌並寄送電子郵件通知。 |
| `config_loader.py` | 核心設定模組。從 BigQuery 動態載入系統設定，失敗時改用環境變數。 |
...
1.  **載入設定**：`config_loader.py` 從 BigQuery `backup_config` 資料表讀取要備份的表單清單。
2.  **查詢同步點**: `bigquery_uploader.py` 查詢 BigQuery 中各表 (`sheet_code`) 的最新時間戳。
3.  **增量擷取**：`ragic_client.py` 依時間戳，從 Ragic API 擷取變動資料。
4.  **資料轉換**：`data_transformer.py` 使用三層欄位對照策略（靜態、動態、自動拼音）轉換資料。
5.  **資料上傳**：`bigquery_uploader.py` 依資料量自動選擇最優上傳策略（Direct MERGE 或 Staging+SP）至 BigQuery。
6.  **結果通知**：`email_notifier.py` 寄送執行結果郵件。

---

## 4. 設定管理（重要）

系統設定優先從 BigQuery 動態載入，允許在不重新部署應用的情況下調整備份目標與欄位對應。環境變數僅作為備援方案。

### 4.1 BigQuery 設定資料表說明

**初次設定**:
必須先在 BigQuery 中執行 `sql/setup_bigquery_config_tables.sql` 腳本來建立以下兩個資料表並填入預設值。
...
- **Schema**:
  ...
  - `sheet_code` (STRING): 內部表單代號（例如 '10', '99'）。
  - `sheet_id` (STRING): Ragic 實際表單路徑（例如 'forms8/3'）。
  - `sheet_name` (STRING): 表單中文名稱。
...
- **管理**：當 Ragic 新增欄位時，在此資料表新增一筆記錄即可被系統自動識別，無需更動程式碼。

---

## 5. 環境變數（備援方案）

僅當 `config_loader.py` 無法從 BigQuery 讀取設定時，系統才會使用以下環境變數。
...
- `load_backup_config()`：從 BigQuery `backup_config` 資料表載入設定。
- `get_sheet_config()`：取得單一表單的設定。
- `clear_cache()`：清除記憶體中的設定快取。

#### `load_config_with_fallback()` 函式
此為系統設定的統一入口，實現「優先 BigQuery，失敗時改用環境變數」的邏輯。
...
2.  **設定 BigQuery**：在 BigQuery 介面執行 `sql/setup_bigquery_config_tables.sql` 腳本，建立 `backup_config` 與 `field_mappings` 資料表。
3.  **部署 Cloud Function**：執行 `scripts/migrate_to_gcp_cf.sh` 腳本。此腳本會讀取環境變數（作為備援）並部署 Cloud Function。
4.  **設定排程**：腳本會自動建立 Cloud Scheduler 以定時觸發 Function。
...
### 流程
1.  **擷取資料**：`fetch_ragic_all.py`
    - 從 Ragic 擷取資料並存為本機 JSON 檔案。
...
### 日常維護
- **修改備份目標**：直接在 BigQuery `backup_config` 資料表中修改 `enabled` 旗標或新增/刪除記錄。
- **新增欄位對應**：當 Ragic 表單新增欄位時，在 `field_mappings` 資料表中增加新的對照規則。

### 故障排除
- **設定未生效**：檢查 `config_loader` 的日誌，確認是從 BigQuery 還是環境變數載入。若從 BigQuery 載入，可呼叫 `clear_cache()` 或重啟 Function 來強制刷新。
