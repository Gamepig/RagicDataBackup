...
- **Gmail 帳號與應用程式密碼**：用於寄送備份結果通知信。基於安全考量，需使用「應用程式密碼」，而非您的 Gmail 登入密碼。
...
1.  登入 Google Cloud 主控台（GCP Console），並選擇您的專案。
2.  在左側導覽列找到並進入 BigQuery。
3.  複製 `sql/setup_bigquery_config_tables.sql` 檔案中的所有文字內容。
4.  在 BigQuery 的「查詢」編輯器中，貼上您剛複製的內容。
5.  按一下「執行」按鈕。

完成後，您會在左側看到 `erp_backup` 資料集，其中包含 `backup_config` 與 `field_mappings` 兩個資料表。這表示系統的「大腦」已經設定好了。

#### 步驟 2：設定環境（Cloud Shell）

1.  在 GCP Console 頁面右上角，按一下「啟用 Cloud Shell」圖示（圖示類似 `>_`）。
2.  一個終端機視窗會在頁面下方開啟。
3.  將本專案的程式碼上傳到 Cloud Shell 環境中。
4.  在終端機中，輸入以下指令（逐行輸入並按 Enter），將您準備好的資訊設定到環境中：
...
腳本會自動完成所有設定。完成後，您的備份系統就安裝完畢並已在排程中（預設為每週一 03:00 執行）。
...
1.  進入 **BigQuery** 服務。
2.  在左側找到 `erp_backup` 資料集，展開並選取 `backup_config` 資料表。
3.  點擊 **「查詢」**，系統會自動產生 `SELECT * FROM ...` 語法。
4.  若要「停用」某個表單的備份（例如「訂單管理」，其 `sheet_code` 為 `50`），請執行以下 SQL 指令：
    ```sql
    UPDATE `您的專案 ID.erp_backup.backup_config`
    SET enabled = false
    WHERE sheet_code = '50';
    ```
...
- **電子郵件通知**：每次備份完成後，系統會寄送一封摘要郵件到您設定的信箱。
- **BigQuery 資料表**：您可以隨時到 BigQuery 中查詢 `ragic_data` 資料表，查看已備份的資料。
  ```sql
  -- 查詢各表單已備份的資料筆數
  SELECT sheet_code, COUNT(*) AS total_records
  FROM `您的專案 ID.erp_backup.ragic_data`
  GROUP BY sheet_code;
  ```
...
- **備份資料沒有更新？**
  ...
  - 到 BigQuery `backup_config` 資料表中檢查對應的表單 `enabled` 欄位是否為 `true`。
